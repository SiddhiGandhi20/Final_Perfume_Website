...
import './AdminExclusivePerfumes.css';

const AdminExclusivePerfumes = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/exclusive_perfumes/${selectedProduct._id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        setExclusivePerfumes((prevPerfumes) =>
  ...
  const handleDeleteClick = async (productId) => {
    try {
      const response = await fetch(`http://localhost:5000/exclusive_perfumes/${productId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        setExclusivePerfumes(exclusivePerfumes.filter(product => product._id !== productId));
      } else {
  ...
  const handleAddNewProduct = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setExclusivePerfumes((prevPerfumes) => [...prevPerfumes, addedProduct]);
        setIsModalVisible(false);
  ...
  return (
    <div className="admin-page">
      ...
      <div className="category-page">
        <h2>Exclusive Perfumes</h2>
        <div className="product-row">
          {exclusivePerfumes.length === 0 ? (
            <p>Loading Exclusive Perfumes...</p>
          ) : (
            exclusivePerfumes.map((product) => (
...
export default AdminExclusivePerfumes;
